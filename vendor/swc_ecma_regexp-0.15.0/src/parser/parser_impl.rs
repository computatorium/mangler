// Mangler modification: syntax-only validation shares the pattern parser.
use super::{
...
impl<'a> LiteralParser<'a> {
    ...
    /// Validate literal grammar and early errors with explicit nesting stacks,
    /// without constructing a recursive AST or limiting decimal quantifiers.
    pub fn validate(self) -> Result<()> {
        self.parser()?.validate()
    }

    fn parser(self) -> Result<PatternParser<'a>> {
