// Mangler patch: classify parameter slices shared by function and arrow parsing.
use swc_common::Span;
use swc_ecma_ast::*;
...
impl IsSimpleParameterList for [Param] {
    fn is_simple_parameter_list(&self) -> bool {
        self.iter().all(|param| matches!(param.pat, Pat::Ident(_)))
    }
}

impl IsSimpleParameterList for [Pat] {
    fn is_simple_parameter_list(&self) -> bool {
        self.iter().all(|pat| matches!(pat, Pat::Ident(_)))
    }
}

impl IsSimpleParameterList for [ParamOrTsParamProp] {
    fn is_simple_parameter_list(&self) -> bool {
        self.iter().all(|param| {
            matches!(
                param,
