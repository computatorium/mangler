...
impl<I: Tokens> Buffer<I> {
    ...
    pub fn bump(&mut self) {
        self.bump_with_identifier_name(false);
    }

    pub fn cur_has_escape(&self) -> bool {
        self.iter
            .read_string(self.cur.span)
            .as_bytes()
            .contains(&b'\\')
    }

    fn bump_with_identifier_name(&mut self, identifier_name: bool) {
        if !identifier_name && self.cur.token.is_word() && self.cur_has_escape() {
            let word = self.cur.token.take_word(self);
            let error = Error::new(
                self.cur.span,
                SyntaxError::EscapeInReservedWord { word: word.clone() },
            );
            if self.iter.ctx().is_reserved_word(&word) {
                self.iter.add_error(error);
            } else if word == "await" && self.iter.ctx().contains(Context::CanBeModule) {
                // A later import/export may classify an ambiguous program as
                // a module after this identifier has already been consumed.
                self.iter.add_module_mode_error(error);
            }
        }
    ...
    pub fn expect_word_token_and_bump(&mut self) -> Atom {
        self.expect_word_and_bump(false)
    }

    /// Consume an IdentifierName, where escaped reserved words are valid.
    /// Keyword and Identifier grammar positions use the ordinary bump path.
    pub fn expect_ident_name_and_bump(&mut self) -> Atom {
        self.expect_word_and_bump(true)
    }

    fn expect_word_and_bump(&mut self, identifier_name: bool) -> Atom {
        let cur = self.cur();
        // Keep the decoded value available until bump validates its grammar
        // position; advancing the token then replaces the buffered value.
        let word = cur.take_word(self);
        self.bump_with_identifier_name(identifier_name);
        word
    }
