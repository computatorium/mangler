...
impl<I: Tokens> Parser<I> {
    ...
    pub(crate) fn parse_module_export_name(&mut self) -> PResult<ModuleExportName> {
        ...
        let module_export_name = if cur == Token::Str {
            ...
            // ModuleExportName requires well-formed Unicode, unlike ordinary
            // string literals and import attribute keys. Wtf8 retains lone
            // surrogates, which cannot be represented as a UTF-8 string.
            if name.value.as_str().is_none() {
                syntax_error!(self, name.span, SyntaxError::InvalidModuleExportName)
            }
    ...
    pub(crate) fn parse_ident_name(&mut self) -> PResult<IdentName> {
        ...
        let w = if cur.is_word() {
            self.input_mut().expect_ident_name_and_bump()
