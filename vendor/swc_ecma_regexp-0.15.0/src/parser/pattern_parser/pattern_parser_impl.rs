// Mangler modification: exact unbounded decimal validation; retain bounded AST parsing.
use std::vec;
...
impl<'a> PatternParser<'a> {
    ...
    fn parse_assertion(&mut self) -> Result<Option<ast::Term>> {
        ...
        let kind = self.consume_lookaround_prefix();
    ...
    fn consume_lookaround_prefix(&mut self) -> Option<ast::LookAroundAssertionKind> {
        if self.reader.eat3('(', '?', '=') {
    ...
    // ```
    fn consume_capturing_prefix(&mut self) -> Result<Option<Option<SpanAtom>>> {
        let checkpoint = self.reader.checkpoint();
    ...
    fn parse_capturing_group(&mut self) -> Result<Option<ast::CapturingGroup>> {
        let span_start = self.reader.offset();
        if let Some(group_name) = self.consume_capturing_prefix()? {
            let disjunction = self.parse_disjunction()?;
            if self.reader.eat(')') {
    ...
    // (? RegularExpressionModifiers - RegularExpressionModifiers : Disjunction[~UnicodeMode, ~UnicodeSetsMode, ?NamedCaptureGroups] )
    // ```
    fn consume_ignore_prefix(&mut self) -> Result<Option<Option<ast::Modifiers>>> {
        if self.reader.eat2('(', '?') {
    ...
    fn parse_ignore_group(&mut self) -> Result<Option<ast::IgnoreGroup>> {
        ...
        if let Some(modifiers) = self.consume_ignore_prefix()? {
            let disjunction = self.parse_disjunction()?;
    ...
    fn consume_quantifier(&mut self) -> Result<Option<((u64, Option<u64>), bool)>> {
        ...
        if self.reader.eat('{') {
            if let Some(min) = self.consume_decimal_digits()? {
                if self.reader.eat('}') {
                    if !self.validation_only && MAX_QUANTIFIER < min.value {
                        return Err(diagnostics::too_large_number_in_braced_quantifier(
                            self.span_factory.create(span_start, self.reader.offset()),
                        ));
                    }

                    return Ok(Some(((min.value, Some(min.value)), is_greedy(&mut self.reader))));
                }
                ...
                if self.reader.eat(',') {
                    if self.reader.eat('}') {
                        ...
                        return Ok(Some(((min.value, None), is_greedy(&mut self.reader))));
                    }

                    if let Some(max) = self.consume_decimal_digits()? {
                        if self.reader.eat('}') {
                            if max.less_than(&min) {
                            ...
                            if !self.validation_only && (MAX_QUANTIFIER < min.value || MAX_QUANTIFIER < max.value) {
                                return Err(diagnostics::too_large_number_in_braced_quantifier(
                                    self.span_factory.create(span_start, self.reader.offset()),
                                ));
                            }

                            return Ok(Some(((min.value, Some(max.value)), is_greedy(&mut self.reader))));
                        }
                    }
                }
            }
    ...
    fn consume_decimal_escape(&mut self) -> Result<Option<u32>> {
        // DecimalEscape starts with NonZeroDigit. A leading zero belongs to
        // CharacterEscape (or legacy octal), even when capture groups exist.
        if self.reader.peek() == Some('0' as u32) {
            return Ok(None);
        }
        // Overflow denotes an out-of-range reference, never a wrapped
        // reference to an unrelated existing capture group.
        Ok(self.consume_decimal_digits()?.map(|digits| {
            u32::try_from(digits.value).unwrap_or(u32::MAX)
        }))
    }
    ...
    fn consume_decimal_digits(&mut self) -> Result<Option<DecimalDigits>> {
        let span_start = self.reader.offset();
        let checkpoint = self.reader.checkpoint();
            ...
            .filter(|&cp| character::is_decimal_digit(cp))
        {
            ...
            if let Some(v) = value.checked_mul(10).and_then(|v| v.checked_add(d)) {
                value = v;
                self.reader.advance();
            } else if self.validation_only {
                // Syntax has no implementation-sized quantifier limit. Full
                // digits remain available for exact range ordering; this value
                // is used only in the AST discarded by LiteralParser::validate.
                value = u64::MAX;
                self.reader.advance();
            } else {
        ...
        if self.reader.checkpoint() != checkpoint {
            return Ok(Some(DecimalDigits {
                value,
                exact: self.validation_only.then(|| self.reader.atom(span_start, self.reader.offset())),
            }));
        }
