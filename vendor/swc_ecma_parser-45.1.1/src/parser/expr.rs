...
impl<I: Tokens> Parser<I> {
    ...
    fn finish_assignment_expr(&mut self, start: BytePos, cond: Box<Expr>) -> PResult<Box<Expr>> {
        ...
        if let Some(op) = self.input().cur().as_assign_op() {
            ...
            ) && self.accept_annex_b_call_target(&cond)
            {
                // The AST has no Call assignment-target variant. Parentheses
                // retain the exact expression without pretending it is simple.
                let paren = match *cond {
    ...
    fn parse_subscript_super(
        ...
        match self.input().cur() {
            ...
            Token::LParen if !no_call => {
                // The TypeScript dialect sets allow_super_outside_method for
                // deferred type checking; it is not an explicit lexical grant.
                let external_super = !self.syntax().typescript()
                    && self.syntax().allow_super_outside_method();
                if !self.allow_super_call() && !external_super {
        ...
        while !self.input().is(Token::RParen) {
            ...
            if self.input_mut().eat(Token::Arrow) && {
                ...
                let body: Box<ArrowFunctionBody> =
                    self.parse_fn_block_or_expr_body(false, false, true, &params)?;
                let span = self.span(start);
    ...
    fn parse_paren_expr_or_arrow_fn(
            ...
            && self.input().is(Token::Colon)
        {
            ...
            if let Some(expr) = self.try_parse_ts(|p| {
                ...
                let body: Box<ArrowFunctionBody> =
                    p.parse_fn_block_or_expr_body(async_span.is_some(), false, true, &params)?;
        ...
        if has_pattern || return_type.is_some() || self.input().is(Token::Arrow) {
            ...
            let body: Box<ArrowFunctionBody> =
                self.parse_fn_block_or_expr_body(async_span.is_some(), false, true, &params)?;
        ...
        let try_parse_arrow_expr = |p: &mut Self, id: Ident, id_is_async| -> PResult<Box<Expr>> {
            ...
            if id_is_async && cur.is_word() && !cur.is_reserved(p.ctx()) {
                if !p.input().had_line_break_before_cur() {
                    ...
                    let body = p.parse_fn_block_or_expr_body(true, false, true, &params)?;
                ...
                p.bump();
                p.record_await_in_arrow_params(&params);
                let body = p.parse_fn_block_or_expr_body(false, false, true, &params)?;
