...
// BoundNames follows only binding positions, never default expressions or
// computed property keys. Parameter and export validation share this recursion.
pub(super) fn for_each_binding_ident(pat: &Pat, visit: &mut impl FnMut(&Ident)) {
    match pat {
        Pat::Ident(i) => visit(&i.id),
        Pat::Array(arr) => {
            for pat in arr.elems.iter().flatten() {
                for_each_binding_ident(pat, visit)
            }
        }
        Pat::Rest(r) => for_each_binding_ident(&r.arg, visit),
        Pat::Object(obj) => {
            for prop in obj.props.iter() {
                match prop {
                    ObjectPatProp::KeyValue(KeyValuePatProp { value, .. })
                    | ObjectPatProp::Rest(RestPat { arg: value, .. }) => {
                        for_each_binding_ident(value, visit)
                    }
                    ObjectPatProp::Assign(AssignPatProp { key, .. }) => visit(&key.id),
                    #[cfg(swc_ast_unknown)]
                    _ => unreachable!(),
                }
            }
        }
        Pat::Assign(a) => for_each_binding_ident(&a.left, visit),
...
impl<I: Tokens> Parser<I> {
    ...
    /// eval
    pub(super) fn check_parameter_binding(
        &mut self,
        pat: &Pat,
        names: &mut Option<HashSet<swc_atoms::Atom>>,
    ) {
        for_each_binding_ident(pat, &mut |ident| self.check_parameter_ident(ident, names));
    }

    fn check_parameter_ident(
        &mut self,
        ident: &Ident,
        names: &mut Option<HashSet<swc_atoms::Atom>>,
    ) {
        self.check_binding_ident(ident);
        if names
            .as_mut()
            .is_some_and(|names| !names.insert(ident.sym.clone()))
        {
            self.emit_err(ident.span, SyntaxError::DuplicateParameter);
        }
    }

    pub(super) fn check_binding_ident(&mut self, ident: &Ident) {
        if self.syntax().typescript() && ident.sym == *"this" {
            return;
        }
        ...
        } else if self.ctx().contains(Context::Strict) && ident.is_reserved_in_strict_bind() {
    ...
    pub(crate) fn parse_constructor_params(&mut self) -> PResult<Vec<ParamOrTsParamProp>> {
        ...
        // Constructors have UniqueFormalParameters even when their outer
        // lexical context permits sloppy ordinary function duplicates.
        let mut names = Some(HashSet::new());
        for parameter in &params {
            match parameter {
                ParamOrTsParamProp::Param(parameter) => {
                    self.check_parameter_binding(&parameter.pat, &mut names);
                }
                ParamOrTsParamProp::TsParamProp(property) => match &property.param {
                    TsParamPropParam::Ident(ident) => {
                        self.check_parameter_ident(&ident.id, &mut names);
                    }
                    TsParamPropParam::Assign(assignment) => {
                        self.check_parameter_binding(&assignment.left, &mut names);
                    }
                },
            }
        }
    ...
    pub(super) fn parse_paren_items_as_params(
        ...
        if self.ctx().contains(Context::Strict) {
            for param in params.iter() {
                self.check_parameter_binding(param, &mut None)
            }
        }
        Ok(params)
    }
