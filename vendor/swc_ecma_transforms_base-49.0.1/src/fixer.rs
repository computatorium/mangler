// Modified by Mangler: retain the AST carrier for Annex B call assignment targets.
...
impl VisitMut for Fixer<'_> {
    ...
    fn visit_mut_assign_target(&mut self, n: &mut AssignTarget) {
        ...
        match n {
            // Parentheses preserve the assignment reference but change
            // IsIdentifierRef, and therefore anonymous RHS naming. They
            // also carry Annex B call targets through this AST.
    ...
    fn visit_mut_export_default_expr(&mut self, node: &mut ExportDefaultExpr) {
        ...
        node.visit_mut_children_with(self);
        match &mut *node.expr {
            Expr::Arrow(..) | Expr::Seq(..) | Expr::Fn(..) | Expr::Class(..) => {
                // Keep an expression from becoming a hoisted default declaration.
                self.wrap(&mut node.expr);
            }
            _ => {}
        };
        self.ctx = old;
...
mod tests {
    ...
    identical!(default_function_expression, "export default (function() {});");
    identical!(default_generator_expression, "export default (function*() {});");
    identical!(
        default_async_function_expression,
        "export default (async function() {});"
    );
    identical!(
        default_async_generator_expression,
        "export default (async function*() {});"
    );
    identical!(default_class_expression, "export default (class {});");
