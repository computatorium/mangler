...
fn parse_ts_this_param<I: Tokens>(p: &mut Parser<I>) -> PResult<Option<Box<TsThisParam>>> {
    if !p.syntax().typescript() || !p.input().is(Token::This) {
        return Ok(None);
    }
    ...
    let is_optional = p.input_mut().eat(Token::QuestionMark);
    if is_optional {
...
impl<I: Tokens> Parser<I> {
        ...
        let f = |p: &mut Self| {
            ...
            let (this_param, params) = p.without_async_arrow_param_await_collection(|p| {
                p.do_inside_of_context(Context::InParameters, |p| {
                    // A non-arrow function's parameters do not inherit the
                    // enclosing static block's Await grammar restriction.
                    p.do_outside_of_context(
                        Context::InFunction.union(Context::InStaticBlock),
                        |p| {
                            if is_async {
                                p.do_inside_of_context(
                                    Context::InAsync,
                                    parse_args_with_generator_ctx,
                                )
                            } else {
                                p.do_outside_of_context(
                                    Context::InAsync,
            ...
            p.check_parameters_after_body(
                params.iter().map(|param| &param.pat),
                body.as_ref(),
                is_async,
                is_generator,
                unique_parameters,
        ...
        self.do_outside_of_context(
            ...
            |p| {
                ...
                if let Some(ident) = &ident {
                    if f.body
                        .as_ref()
                        .is_some_and(|body| has_use_strict(&body.stmts).is_some())
                    {
                        // Name parsing already applied declaration/expression
                        // Await and Yield grammar. A body directive adds only
                        // strictness, not the function's own async/generator
                        // parameter restrictions (async function await(){} is
                        // a valid declaration in an ordinary Script).
                        p.do_outside_of_context(
                            Context::InAsync
                                .union(Context::InGenerator)
                                .union(Context::InStaticBlock),
                            |p| p.strict_mode(|p| p.check_binding_ident(ident)),
                        );
                    }
                }
                Ok((ident, f))
        ...
        F: FnOnce(&mut Self) -> PResult<Vec<Param>>,
    {
        ...
        let function = self.do_inside_of_context(Context::AllowDirectSuper, |p| {
            p.do_outside_of_context(Context::InClassField, |p| {
                p.parse_fn_args_body(decorators, start, parse_args, is_async, is_generator, true)
            })
        })?;
    ...
    fn check_parameters_after_body<'p>(
        &mut self,
        params: impl Iterator<Item = &'p Pat> + Clone,
        ...
        context.remove(Context::InStaticBlock);
        context.set(Context::InAsync, is_async);
        context.set(Context::InGenerator, is_generator);
        context.set(
            Context::Strict,
            previous.contains(Context::Strict)
                || body.is_some_and(|body| has_use_strict(&body.stmts).is_some()),
        );
        self.set_ctx(context);
        // Sloppy simple function parameters may repeat, including async and
        // generator functions. Methods and arrows have a unique grammar.
        // Default expressions and computed keys are not BoundNames.
        let unique = unique_parameters
            || context.contains(Context::Strict)
            || params
                .clone()
                .any(|parameter| !matches!(parameter, Pat::Ident(_)));
        let mut names = unique.then(HashSet::new);
        for parameter in params {
            self.check_parameter_binding(parameter, &mut names);
        }
        self.set_ctx(previous);
    }
    ...
    pub(crate) fn parse_fn_block_or_expr_body(
        ...
        let is_simple_parameter_list = params.is_simple_parameter_list();
                    ...
                    p.parse_block(false)
                        .map(|block_stmt| {
                            if !is_simple_parameter_list {
                                if let Some(span) = has_use_strict(&block_stmt.stmts) {
        ...
        self.check_parameters_after_body(
            params.iter(),
            match &*body {
    ...
    fn make_property(
        ...
        self.do_inside_of_context(Context::IncludeInExpr.union(Context::InClassField), |p| {
            ...
            let value = p.with_super_call(false, |p| {
                p.without_async_arrow_param_await_collection(|p| {
    ...
    fn parse_static_block(&mut self, start: BytePos) -> PResult<ClassMember> {
        let body = self.do_inside_of_context(
            ...
            |p| p.with_super_call(false, |p| p.parse_block(false)),
        )?;
    ...
    fn parse_class_member_with_is_static(
        ...
        if self.input_mut().eat(Token::Asterisk) {
            ...
            if is_constructor(&key) && !is_static {
        ...
        if self.is_class_method() {
            ...
            let is_constructor = is_constructor(&key) && !is_static;
            ...
            if is_constructor {
                ...
                self.set_allow_super_call(self.ctx().contains(Context::HasSuperClass));
                let ctor_sig_and_body =
                    (|| -> PResult<(Vec<ParamOrTsParamProp>, Option<FunctionBody>)> {
                        expect!(self, Token::LParen);
                        // Constructor parameters have their own non-async,
                        // non-generator grammar even inside an outer function
                        // or class static block.
                        let params = self.do_outside_of_context(
                            Context::InStaticBlock
                                .union(Context::InAsync)
                                .union(Context::InGenerator),
    ...
    fn parse_class_member(&mut self) -> PResult<ClassMember> {
        ...
        let static_token = {
            let start = self.cur_pos();
            // An escaped `static` is an IdentifierName, never the modifier.
            if self.input().is(Token::Static)
                && !self.input().cur_has_escape()
                && self.input_mut().eat(Token::Static)
            {
                Some(self.span(start))
            } else {
                None
...
fn has_use_strict(statements: &[Stmt]) -> Option<Span> {
    statements
        .iter()
