...
impl<I: Tokens> Parser<I> {
    ...
    // SuperCall is inherited only by arrows. Restore on both success and error,
    // including speculative parses that backtrack over a callable boundary.
    fn with_super_call<T>(
        &mut self,
        allowed: bool,
        parse: impl FnOnce(&mut Self) -> PResult<T>,
    ) -> PResult<T> {
        let previous = self.allow_super_call;
        self.allow_super_call = allowed;
        let result = parse(self);
        self.allow_super_call = previous;
        result
    }
...
impl<I: Tokens> Parser<I> {
    ...
    fn report_duplicate_exports(&mut self, body: &[ModuleItem]) {
        let mut exported = FxHashMap::<Wtf8Atom, Span>::default();
        for item in body {
            let ModuleItem::ModuleDecl(module_decl) = item else {
                continue;
            };
    ...
    fn record_exported_name(
        &mut self,
        exported: &mut FxHashMap<Wtf8Atom, Span>,
        name: Wtf8Atom,
        span: Span,
    ) {
        if exported.insert(name, span).is_some() {
            self.emit_err(span, SyntaxError::DuplicateExport);
        }
    }

    fn record_module_export_name(
        &mut self,
        exported: &mut FxHashMap<Wtf8Atom, Span>,
        ...
        match name {
            ModuleExportName::Ident(ident) => {
                self.record_exported_name(exported, ident.sym.clone().into(), ident.span);
            }
            ModuleExportName::Str(string) => {
                self.record_exported_name(exported, string.value.clone(), string.span);
            }
        }
    }

    fn collect_decl_exported_names(
        &mut self,
        decl: &Decl,
        exported: &mut FxHashMap<Wtf8Atom, Span>,
    ) {
        match decl {
            Decl::Class(class_decl) => {
                self.record_exported_name(
                    exported,
                    class_decl.ident.sym.clone().into(),
            ...
            Decl::Fn(fn_decl) => {
                self.record_exported_name(
                    exported,
                    fn_decl.ident.sym.clone().into(),
                    fn_decl.ident.span,
                );
            }
            Decl::Var(var_decl) => {
                for declarator in &var_decl.decls {
                    pat::for_each_binding_ident(&declarator.name, &mut |ident| {
                        self.record_exported_name(exported, ident.sym.clone().into(), ident.span);
                    });
                }
            }
    ...
    fn collect_exported_names(
        ...
        match decl {
                ...
                for spec in &named_export.specifiers {
                    match spec {
                        ExportSpecifier::Named(named) => {
                            ...
                            self.record_module_export_name(exported, export_name);
                        }
                        ExportSpecifier::Default(default) => {
                            self.record_exported_name(
                                exported,
                                default.exported.sym.clone().into(),
                                default.exported.span,
                            );
                        }
                        ExportSpecifier::Namespace(namespace) => {
                            self.record_module_export_name(exported, &namespace.name);
                        }
                        #[cfg(swc_ast_unknown)]
                        _ => {}
                    }
            ...
            ModuleDecl::ExportDefaultExpr(default_expr) => {
                self.record_exported_name(exported, Wtf8Atom::from("default"), default_expr.span);
            }
            ModuleDecl::ExportDefaultDecl(default_decl) => {
                self.record_exported_name(exported, Wtf8Atom::from("default"), default_decl.span);
            }
    ...
    /// Annex B.3.9 permits only ordinary calls, in non-strict JavaScript, as
    /// runtime-invalid assignment targets. Keep this separate from simple
    /// targets: destructuring and logical assignments require the latter.
    fn accept_annex_b_call_target(&mut self, expr: &Expr) -> bool {
        if self.input().syntax().typescript() || self.ctx().contains(Context::Strict) {
            return false;
        }
        let mut target = expr;
        while let Expr::Paren(paren) = target {
            target = &paren.expr;
        }
        if !matches!(target, Expr::Call(call) if matches!(call.callee, Callee::Expr(_))) {
            return false;
        }
        // An unambiguous Program may become a Module at a later export/import.
        self.emit_strict_mode_err(expr.span(), SyntaxError::InvalidAssignTarget);
        true
    }

    pub fn check_assign_target(&mut self, expr: &Expr, deny_call: bool) {
        if !deny_call && self.accept_annex_b_call_target(expr) {
            return;
        }
    ...
    pub fn parse_prop_name(&mut self) -> PResult<PropName> {
            ...
            && peek!(self).is_some_and(|peek| peek == Token::At)
        {
            ...
            let key = self.input_mut().expect_ident_name_and_bump();
            PropName::Str(Str {
                span: self.span(start),
            ...
            let w = self.input_mut().expect_ident_name_and_bump();
