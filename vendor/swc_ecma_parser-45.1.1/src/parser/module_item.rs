...
impl<I: Tokens> Parser<I> {
    ...
    /// Import/export attributes share one grammar and early-error check.
    fn parse_import_attributes(&mut self) -> PResult<Option<Box<ObjectLit>>> {
        if !self.input().syntax().import_attributes()
            || !(self.input_mut().eat(Token::With)
                || (!self.input().had_line_break_before_cur() && self.input_mut().eat(Token::Assert)))
        {
            return Ok(None);
        }
        let Expr::Object(attributes) = self.parse_object_expr()? else { unreachable!() };
        let mut keys = std::collections::HashSet::<swc_atoms::Wtf8Atom>::new();
        for property in &attributes.props {
            let PropOrSpread::Prop(property) = property else {
                syntax_error!(self, attributes.span, SyntaxError::InvalidImportAttributes)
            };
            let Prop::KeyValue(property) = property.as_ref() else {
                syntax_error!(self, attributes.span, SyntaxError::InvalidImportAttributes)
            };
            let key = match &property.key {
                PropName::Ident(name) => name.sym.clone().into(),
                PropName::Str(name) => name.value.clone(),
                _ => syntax_error!(self, attributes.span, SyntaxError::InvalidImportAttributes),
            };
            if !matches!(&*property.value, Expr::Lit(Lit::Str(_))) || !keys.insert(key) {
                syntax_error!(self, attributes.span, SyntaxError::InvalidImportAttributes)
            }
        }
