// Mangler modification: validate RegExp grammar through the shared Rust parser.
//! ECMAScript lexer.
 ...
 start = {}, end = {}",
    ...
    fn read_escaped_char(&mut self, in_template: bool) -> LexResult<Option<CodePoint>> {
        ...
        let c = match c {
            ...
            // Decimal escapes are invalid in template cooked strings, even
            // though Annex B permits them in non-strict ordinary strings.
            '8' | '9' if in_template => {
                self.bump(1);
    ...
    fn read_regexp(&mut self, start: BytePos) -> LexResult<Token> {
        ...
        while let Some(c) = self.cur() {
            ...
            if c.is_line_terminator()
                || (c == LS_OR_PS_FIRST && matches!(self.cur_as_char(), Some('\u{2028}' | '\u{2029}')))
            {
                let span = self.span(start);
        ...
        // The flag token is an IdentifierPart sequence, but escapes are forbidden
        // by RegExp grammar. Retain raw source for the shared pattern/flag parser.
        if self.cur().is_some_and(|c| c.is_ident_start()) {
            self.read_word_as_str_with()?;
        }
        // SAFETY: delimiters and the completed flag token are UTF-8 boundaries
        // inside the input; read_regexp has scanned both source ranges above.
        let pattern = unsafe { self.input_slice_str(start + BytePos(1), exp_end) };
        let flags = unsafe { self.input_slice_str(exp_end + BytePos(1), self.cur_pos()) };
        if let Err(error) = swc_ecma_regexp::LiteralParser::new(
            pattern,
            Some(flags),
        ...
        .validate()
        {
            return Err(crate::error::Error::new(
                self.span(start),
                SyntaxError::InvalidRegExp(error.to_string()),
            ));
        }
    ...
    fn read_keyword_with(&mut self, convert: fn(&str) -> Option<Token>) -> LexResult<Token> {
        ...
        if has_escape {
            // Escaped words are IdentifierNames, not keyword spellings. Keep
            // the decoded value for both property names and reserved checks,
            // matching the first-character-escape read_ident_unknown path.
            self.update_token_flags(|flags| *flags |= TokenFlags::UNICODE);
            let atom = self.atom(s);
            return Ok(Token::unknown_ident(atom, self));
        }
        if let Some(word) = convert(s.as_ref()) {
            // Escaped reserved words are valid IdentifierNames. The parser's
            // token-consumption seam rejects them in keyword/Identifier uses,
            // once the grammar position is known (including buffered lookahead).
            Ok(word)
        } else {
            let atom = self.atom(s);
