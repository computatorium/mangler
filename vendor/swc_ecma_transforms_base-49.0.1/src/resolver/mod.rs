// Modified by Mangler: async/generator block functions remain lexical.
...
impl VisitMut for Hoister<'_, '_> {
    ...
    fn visit_mut_fn_decl(&mut self, node: &mut FnDecl) {
        ...
        if self.in_block {
            // Annex B variable aliases apply only to ordinary sloppy functions.
            if self.resolver.strict_mode || node.function.is_async || node.function.is_generator {
                return;
            }
