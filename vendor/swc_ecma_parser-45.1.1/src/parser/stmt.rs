// Modified by Mangler: for-var initializers explicitly use the NoIn grammar.
use swc_atoms::atom;
...
impl<I: Tokens> Parser<I> {
    ...
    pub(crate) fn parse_var_stmt(&mut self, for_loop: bool) -> PResult<Box<VarDecl>> {
        ...
        loop {
            ...
            let decl = if should_include_in {
                ...
                self.disallow_in_expr(|p| p.parse_var_declarator(for_loop, kind))
            }?;
    ...
    fn parse_using_decl(
        ...
        let declaration = self.parse_using_declaration_list(start, is_await)?;
        self.expect_general_semi()?;
        Ok(Some(declaration))
    }

    // The same resource bindings serve a statement and the NoIn loop head.
    // A C-style loop consumes its mandatory semicolon at the call site.
    fn parse_using_declaration_list(
        &mut self,
        start: BytePos,
        is_await: bool,
    ) -> PResult<Box<UsingDecl>> {
        ...
        let mut names = std::collections::HashSet::with_capacity(decls.len());
        for decl in &decls {
            match decl.name {
                Pat::Ident(ref name) if name.sym != *"let" && names.insert(name.sym.clone()) => {}
                _ => {
    ...
    pub(super) fn parse_for_head(&mut self) -> PResult<TempForHead> {
        ...
        let start = self.cur_pos();
        let await_using_keyword = self.input().is(Token::Await)
            && !self.input_mut().has_linebreak_between_cur_and_peeked();
        let init = self.disallow_in_expr(Self::parse_for_head_prefix)?;

        // VarDeclOrExpr has no resource variant. Retain the genuine declaration
        // in a loop scope instead of pretending it is an ordinary const.
        let resource_head = self.syntax().explicit_resource_management()
            && !self.input().had_line_break_before_cur()
            && (init.is_ident_ref_to("using")
                || (await_using_keyword
                    && init
                        .as_await_expr()
                        .is_some_and(|expr| expr.arg.is_ident_ref_to("using"))));
        if resource_head
            && self.is_ident_ref()
            && peek!(self)
                .is_some_and(|token| matches!(token, Token::Eq | Token::Comma | Token::Semi))
        {
            let declaration = self.disallow_in_expr(|parser| {
                parser.parse_using_declaration_list(start, await_using_keyword)
            })?;
            expect!(self, Token::Semi);
            let TempForHead::For { test, update, .. } = self.parse_normal_for_head(None)? else {
                unreachable!()
            };
            return Ok(TempForHead::ForUsing {
                declaration,
                test,
                update,
            });
        ...
        // Reuse the resource prefix/no-line-terminator classification. Unlike
        // `using of`, `await using of` may declare a binding named `of`.
        let is_using_decl = resource_head
            && (!self.input().is(Token::Of) || await_using_keyword)
            && peek!(self).is_some_and(|token| matches!(token, Token::Of | Token::In));
        ...
        if cur == Token::Of || cur == Token::In {
            ...
            let pat = if self.accept_annex_b_call_target(&init) {
                // Suspension lowering converts this pattern to AssignTarget.
                // As for ordinary assignments, Paren retains a Call in the
                // existing AST without inventing a simple Call target variant.
                let paren = match *init {
                ...
                Pat::Expr(Box::new(Expr::Paren(paren)))
            } else {
                self.reparse_expr_as_pat(PatType::AssignPat, init)?
            };
    ...
    fn parse_for_stmt(&mut self) -> PResult<Stmt> {
        ...
        Ok(match head {
            ...
            TempForHead::ForUsing {
                ...
                // Using is constant: there is one environment and one disposal
                // stack for the entire loop, with no per-iteration copies.
                BlockStmt {
                    span,
    ...
    fn parse_labelled_stmt(&mut self, l: Ident) -> PResult<Stmt> {
        self.do_inside_of_context(Context::IsBreakAllowed, |p| {
            p.do_outside_of_context(Context::AllowUsingDecl, |p| {
                ...
                let mut body = Box::new(if p.input().is(Token::Function) {
                ...
                // Resource-head normalization adds a scope around the loop.
                // Labels must still target that loop, including adjacent labels.
                // Equal block/terminal-loop spans certify our private synthetic
                // shape: source braces necessarily have a different extent.
                if let Stmt::Block(block) = body.as_mut() {
                    if block.stmts.len() == 2
                        && matches!(block.stmts[0], Stmt::Decl(Decl::Using(..)))
                    {
                        let mut terminal = &block.stmts[1];
                        while let Stmt::Labeled(label) = terminal {
                            terminal = &label.body;
                        }
                        if matches!(terminal, Stmt::For(loop_) if loop_.span == block.span) {
                            let inner = block.stmts.pop().unwrap();
                            block.stmts.push(
                                LabeledStmt {
                                    span: p.span(start),
                                    label: l,
