...
fn module_export_names_require_well_formed_unicode() {
    for source in [
        ...
        r#"export { value as "\uD83D" };"#,
        r#"export { "\uD83D" } from 'm';"#,
        r#"export { "valid" as "\uD83D" } from 'm';"#,
        r#"export * as "\uD83D" from 'm';"#,
        r#"import { "\uDC00" as value } from 'm';"#,
    ] {
        test_parser(source, Default::default(), |parser| {
            let error = parser.parse_module().expect_err("unpaired module name");
            assert_eq!(error.kind(), &SyntaxError::InvalidModuleExportName);
            Ok(())
        });
    }
    for source in [
